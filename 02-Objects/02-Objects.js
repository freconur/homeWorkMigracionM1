...
function crearClaseLibro() {
  class Libro {
    ...
    getAlcance() {
      ...
      let arraySinRepetir = [...new Set(array)];
      arraySinRepetir.forEach( e => {
        suma = suma + 1
      })
      return suma
    }
...
const printStaff = function (objeto) {
  ...
  // const hog = hogwarts.staff
  // Tu código aca:
  let array = []
  
  Object.keys(objeto.staff).map(key => {
      let value = objeto.staff[key]
      let newString = 'The' + " " + key + " " + 'is' + " " + value.name
